...
export default function PlaylistsTab() {
  ...
  const setActivePlaylist = async (id: string) => {
    ...
    if (error) { toast.error("Failed to set active playlist"); return; }
  ...
  const duplicatePlaylist = async (sourceId: string) => {
    ...
    if (!source) return;
    const { data: newPl, error } = await supabase.from("playlists").insert({ name: `${source.name} (Copy)` }).select().single();
    if (error || !newPl) { toast.error("Failed to duplicate"); return; }
  ...
  const deletePlaylist = async (id: string) => {
    ...
    if (pl?.is_active) { toast.error("Cannot delete the active playlist"); return; }
  ...
  const copySlideToPlaylist = async (slide: Slide, targetPlaylistId: string) => {
    ...
    if (error) { toast.error("Failed to copy"); return; }
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        if (error) toast.error("Failed to save slide");
  ...
  return (
    <div className="space-y-4 pb-20">
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          {playlists.find(p => p.is_active) && (
            <span className="ml-auto text-[10px] font-bold uppercase tracking-wider bg-primary/15 text-primary px-2 py-0.5 rounded-full">
              Live: {playlists.find(p => p.is_active)?.name}
            </span>
          )}
        </div>
        <div className="flex gap-2">
          <Input placeholder="New playlist name…" value={newPlaylistName} onChange={(e) => setNewPlaylistName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && createPlaylist()} className="h-9 text-sm" />
        ...
        {playlists.length > 0 && (
          <div className="flex gap-2 items-center">
            <Select value={selectedPlaylistId} onValueChange={setSelectedPlaylistId}>
              <SelectTrigger className="h-9 text-sm flex-1"><SelectValue placeholder="Select playlist" /></SelectTrigger>
              <SelectContent>
                {playlists.map((p) => (
            ...
            <Button size="sm" className="h-9 gap-1 shrink-0" variant={isSelectedActive ? "secondary" : "default"} disabled={isSelectedActive} onClick={() => setActivePlaylist(selectedPlaylistId)}>
              <Check className="w-3.5 h-3.5" /> {isSelectedActive ? "Active" : "Set Active"}
            </Button>
          </div>
        )}
      </div>
      ...
      <label className="flex flex-col items-center justify-center gap-2 p-6 border-2 border-dashed border-primary/30 rounded-xl bg-primary/5 cursor-pointer hover:bg-primary/10 transition-colors">
        ...
        <span className="text-sm font-medium text-primary">{uploading ? "Uploading..." : "Tap to upload images or videos"}</span>
          ...
          {slides.map((slide, index) => (
            <div key={slide.id} className="bg-card rounded-xl border border-border overflow-hidden shadow-sm">
              <div className="aspect-video bg-muted relative">
                ...
                <span className="absolute top-1.5 left-1.5 bg-foreground/70 text-background text-[10px] font-bold px-1.5 py-0.5 rounded-md">#{index + 1}</span>
                {slide.media_type === "video" && (
                ...
                <div className="flex items-center gap-1">
                  ...
                  {otherPlaylists.length > 0 && (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuSub>
                          <DropdownMenuSubTrigger><Copy className="w-4 h-4 mr-2" /> Copy to…</DropdownMenuSubTrigger>
      ...
      {slides.length > 0 && (
        <div className="fixed bottom-16 left-0 right-0 p-4 bg-card/80 backdrop-blur-xl border-t border-border z-40">
          <Button onClick={handleSync} disabled={saving} className="w-full h-12 text-base font-semibold gap-2">
            <Save className="w-5 h-5" /> {saving ? "Syncing..." : "Sync to Display"}
          </Button>
        </div>
      )}
